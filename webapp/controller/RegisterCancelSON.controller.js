sap.ui.define(
    ...
    "gestioneabilitazioneeson.controller.RegisterCancelSON",
    {
        formatter: formatter,
        _beneficiario:null,
        ...
        onInit: function () {
            ...
            self.getRouter().getRoute("registerCancelSON").attachPatternMatched(self._onObjectMatched, self);
        ...
        _onObjectMatched: function (oEvent) {
            ...
            if(checkList && checkList.length === 1){
                ...
                self.fillWizard(checkList[0],"gestioneabilitazioneeson.view.RegisterCancelSON");
            }
            setTimeout(() => {                
        ...
        onNavBack:function(oEvent){
            var self = this;
            self.resetForBack("btnRegisterCancelSON",true);
            self.resetLog();
            ...
            switch(key){
                case oBundle.getText("btnRegisterCancelSON"):
                    var oDialog = new sap.m.Dialog({
                        title: oBundle.getText("btnRegisterCancelSON"),
                        state: "Warning",
                        type: "Message",
                        content: [
                          new sap.m.Text({
                            text: oBundle.getText("msgRegisterCancel"),
                          }),
                        ],
                        beginButton: new sap.m.Button({
                          ...
                          type: "Emphasized",
                          press: function () {
                            self.callDeep(OPERATION_TYPE_REGISTER_CANCEL);
                            oDialog.close();
                          }
                        }),
            ...
            if(key !== oBundle.getText("btnRegisterCancelSON")){
                ...
                if (selectedItem !== null) {
                    ...
                    if(action === oBundle.getText("btnDetail")){
                        ...
                        self.getView().getModel(DETAIL_MODEL).setProperty("/buttonVisible",false);
                        self.fillWizard(oItem,"gestioneabilitazioneeson.view.RegisterCancelSON");
                    }
        ...
        callDeep: function (operationType) {
            ...
            oDataModel.create("/" + URL_DEEP, entityRequestBody, {
              success: function (result) {
                ...
                if (isSuccess) {      
                    var successContent= oBundle.getText("operationOK"),
                        text="",
                        fix ="/nRichiesta di annullamento n. ",
                        fix2 ="registrata per il SON N. "; 
                    
                    for(var i=0; i<checklist.length;i++){
                        text = text + fix + (i+1).toString() + fix2 + checklist[i].Zchiavesop;                        
                    }
                    successContent = successContent+text;
                    sap.m.MessageBox.success(successContent, {
